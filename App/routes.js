...
router.post(`${API}/new-user`, async (req, res) =>
  res.send(await userController.registerUser(req))
);
router.get(`${API}/log`, async (req, res) =>
  res.send(await trainingController.getTrainings(req))
);
router.post(`${API}/add`, async (req, res) =>
  res.send(await trainingController.registerTraining(req))
);
